...
class CryptoPricePredictor:
    ...
    def __init__(self, symbols: list, predict_days: int = 5):
        """
        Initialize the predictor with symbols and prediction parameters.
        
        Args:
            symbols (list): A list of cryptocurrency symbols (e.g., ['BTC-USD', 'ETH-USD']).
...
def save_predictions_to_csv(df_results: pd.DataFrame, filename: str, predict_days: int = 5):
    """
    Save prediction results to CSV file and display summary.
    
    Args:
        df_results (pd.DataFrame): DataFrame with prediction results.
        filename (str): Output CSV filename.
        predict_days (int): Number of days predicted.
    """
    if df_results.empty:
    ...
    if not output_df.empty:
        ...
        print(f"   Estimated Price in {predict_days} days: {best_crypto['Estimated Price']:.2f}")
        print(f"   Expected {predict_days}-day gain: {best_crypto['Combined Gain %']:.2f}%")


def run_prediction_flow(symbols: list, predict_days: int = 5) -> pd.DataFrame:
    """
    The main tool function for price prediction.
    This function can be imported and used in a larger workflow (e.g., LangGraph).
    
    Args:
        symbols (list): A list of cryptocurrency symbols to predict.
        predict_days (int): The number of days into the future to forecast.

    Returns:
        pd.DataFrame: A dataframe containing the detailed prediction results.
    """
    predictor = CryptoPricePredictor(symbols=symbols, predict_days=predict_days)
    predictor.predict_all_cryptos()
    return predictor.get_results_dataframe()
...
def main():
    ...
    output_csv_path = '../data/predict.csv'
    predict_days = 5

    try:
        symbols = pd.read_csv(symbols_csv_path)['symbol'].tolist()
    except FileNotFoundError:
        print(f"Warning: Symbols file not found at {symbols_csv_path}. Using fallback list.")
        symbols = ['BTC-USD', 'ETH-USD', 'XRP-USD'] # Fallback symbols
    
    # Run the prediction workflow
    results_df = run_prediction_flow(symbols=symbols, predict_days=predict_days)
    
    # Save results
    if not results_df.empty:
        save_predictions_to_csv(results_df, output_csv_path, predict_days)
